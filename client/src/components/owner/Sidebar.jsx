...
const Sidebar = () => {
  ...
  const updateImage = async () => {
    try {
      const formData = new FormData(); // ✅ use variable name
      formData.append("image", image); // ✅ use correct .append
          ...
          <img
            src={
              image
                ...
                : user?.image || "https://images.app.goo.gl/yuhGkNd2STTj7ZRS8"
            }
            alt=""
      ...
      <div className="w-full">
        {ownerMenuLinks.map((link, index) => (
          <NavLink
            key={index}
            to={link.path}
            className={`relative flex items-center gap-2 w-full py-4 first:mt-6 ${
